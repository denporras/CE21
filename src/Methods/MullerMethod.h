...
complex<T> MullerMethod<T>::getRoot(polynomial<T> &poly,T xr,T h){
	...
	while((abs(dx3) > abs(EPS*x3)) || (i > MAX_ITERATION)){
		...
		h0 = x1 - x0;
		h1 = x2-x1;
		d0 = (this->evaluatePolynomial(poly,x1)-this->evaluatePolynomial(poly,x0))/h0; //(f(x1)-f(x0))/h0
		d1 = (this->evaluatePolynomial(poly,x2)-this->evaluatePolynomial(poly,x1))/h1; //(f(x2)-f(x1))/h1
		a = (d1-d0)/(h1+h0);
		b = a*h1 + d1;
